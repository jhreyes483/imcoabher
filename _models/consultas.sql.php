...
//======================================================
// CPAGOS
public function  verPagos($param){
  // Controller::ver($param,1);
   $sql = 'SELECT  P.fecha , P.hora, 
   U.nom1, U.nom2, U.ape1, U.ape2,
   P.actor , P.estado , P.valor,
   TE.nom_egreso,
   PP.nom_pago_prog, PP.estado,
   P.id_pago , MP.nombre_m
   FROM pago P
   LEFT JOIN usuario U ON U.ID_us = P.fk_user
   LEFT JOIN tipo_egreso TE ON TE.id_egreso = P.fk_egreso 
   LEFT JOIN pago_prog PP ON PP.fk_pago = P.id_pago
...
public function verPagosT($param = []){
   $sqlAnex = implode(' ', $param);
   $sql = 'SELECT  P.fecha , "", 
   U.nom1, U.nom2, U.ape1, U.ape2,
   P.actor , P.estado , P.valor,
   TE.nom_egreso,
...
//   foreach($datosModel as $i =>  $d ){
     // $pass_cifrado = password_hash($d[1], PASSWORD_DEFAULT);
      $consulta->bindValue( ':correo',       $d[0] , PDO::PARAM_STR );
   // $consulta->bindValue( ':pass',        $d[1] , PDO::PARAM_STR );
      $consulta->bindValue(':ID_acronimo',  $d[2], PDO::PARAM_STR  );
      $pass = $d[1];
   $consulta->execute();
...
/*
  ...
  //-------------------------------------------------------------------------
   ...
   public function actualizarDatosUsuario($id, $a){ 
      ...
      fecha = ?,  correo = ?, FK_tipo_doc = ?
      WHERE ID_us = ?";
      $insertar = $this->db->prepare($sql);
      $bool = $insertar->execute([$a[0], $a[1], $a[2], $a[3], $a[4], $a[5], $a[8], $a[9], $id]);       
...
//CFACTURA
   ...
   public function facturar($a){
     ...
     public function updateCantidadProducto( $a){
        $sql = 'UPDATE producto SET stok_prod = '.$a[0].' WHERE producto.ID_prod = "'.$a[1].'"';
        $consulta = $this->db->prepare($sql);
...
//===================================================
//CANUNCIO
public function insertAnuncio(array $a ){
   $sql = "INSERT INTO `anuncio` ( `nombre`, `descript`, `fecha`, `FK_tipoA`) 
   VALUES ( :nombre, :descript, :fecha ,:FK_tipoA )";
   $stm = $this->db->prepare($sql);
   $stm->bindValue(":nombre",   $a[0], PDO::PARAM_STR );
   $stm->bindValue(":descript", $a[1], PDO::PARAM_STR );
   $stm->bindValue(":fecha",    $a[2], PDO::PARAM_STR );
   $stm->bindValue(":FK_tipoA", $a[3], PDO::PARAM_INT );
   return $stm->execute();
}

public function updateAnuncio(array $a ){
   $sql = "UPDATE `anuncio` 
   SET `nombre` = ':nombre', `descript` = ':descript' , `fecha` = ':fecha' , `FK_tipoA` = ':FK_tipoA'
   WHERE `anuncio`.`ID_a` = :ID_a";
   $stm = $this->db->prepare($sql);
   $stm->bindValue(":nombre",   $a[0], PDO::PARAM_STR );
   $stm->bindValue(":descript", $a[1], PDO::PARAM_STR );
   $stm->bindValue(":fecha",    $a[2], PDO::PARAM_STR );
   $stm->bindValue(":FK_tipoA", $a[3], PDO::PARAM_INT );
   $stm->bindValue(":ID_a",     $a[4], PDO::PARAM_INT );
   return $stm->execute();
}

public function verAnuciosIndex( $tipo ){
...
public function eliminarTelefono($idg){
   ...
   public function inserDireccion($a){
    //  Controller::ver($a,1,'eee');
      $sql = 'INSERT INTO direccion (dir, CF_us, CF_tipo_doc, CF_rut, FK_barrio, FK_Localidad, FK_Ciudad) 
      VALUES (?,?,?,?,?,?,?)';
         $stm= $this->db->prepare($sql);
         $stm->bindValue( 1, $a[0], PDO::PARAM_STR );
         $stm->bindValue( 2, $a[1], PDO::PARAM_STR );
         $stm->bindValue( 3, $a[2], PDO::PARAM_STR );
         $stm->bindValue( 4, NULL, PDO::PARAM_INT );
         $stm->bindValue( 5, $a[4], PDO::PARAM_INT );
         $stm->bindValue( 6, $a[5], PDO::PARAM_INT );
...
//CNOTIFICACION
   ...
   public function insertPago(array $a ){
      ...
      ( fecha, actor, estado, valor, fk_user, fk_egreso, fk_motivo)
      VALUES (?,?,?,?,?,?,?)';
      $stm = $this->db->prepare($sql);
      $stm->bindValue(1,$a[0], PDO::PARAM_STR);
      $stm->bindValue(2,$a[1], PDO::PARAM_STR);
      ...
      $stm->bindValue(5,$a[4], PDO::PARAM_STR);
      $stm->bindValue(6,$a[5], PDO::PARAM_INT);
      $stm->bindValue(7,$a[6], PDO::PARAM_INT);
      return  $stm->execute();
   }
